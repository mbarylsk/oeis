...
// C++ source code that generates A294185.
...
long long unsigned int no_of_unique_lesser_twin_primes_in_gp (long long unsigned int n);
bool is_prime (long long unsigned int n);
bool is_lesser_twin_prime (long long unsigned int n);
...
int main()
{
	...
	{
		cout << no_of_unique_lesser_twin_primes_in_gp(2*i) << ", ";
	}
	return 0;
}
...
long long unsigned int no_of_unique_lesser_twin_primes_in_gp (long long unsigned int n)
{
	...
	// counts distinct lesser twin primes in Goldbach partitions
	long long unsigned int c = 0;
	for (it=setprimes.begin(); it!=setprimes.end(); ++it)
	{
		if (is_lesser_twin_prime(*it)) c++;
	}
	return c;
}
